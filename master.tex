...
% ------------------------------------------------------------------------------
  ...
  \section{Slučajni grafovi}
	...
	%U klasama \emph{ran2}, \emph{ran10} i \emph{ransq} složenost
	%pretprocesiranja invarijante grafa najkraćeg puta ($O(nm)$ za $n$ čvorova i
	%$m$ grana) dominira. Pojačanje procedure profinjavanja je u ovom slučaju
	%zanemarljivo - za skoro sve primere se i bez upotrebe invarijante grafa
	%generiše jedan čvor stabla pretrage.
  ...
  \section{Grafovi nekih kombinatornih objekata}
    ...
    \begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[color=white, only marks, mark=x]
				table{data/had-bad.csv};
